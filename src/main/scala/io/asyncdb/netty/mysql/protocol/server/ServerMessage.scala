...
object ServerMessage {
  ...
  implicit val serverMsgDecoder: Decoder[ServerMessage] = new Decoder[ServerMessage] {

    def decodeMsg[A](buf: ByteBuf, charset: Charset)(implicit ae: Decoder[A]) = {
      ae.decode(buf, charset)
    }

    def decode(buf: ByteBuf, charset: Charset) = {
      val head = buf.getByte(buf.readerIndex()).toByte
        (head: @switch) match {
