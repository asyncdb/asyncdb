...
package object protocol {
  implicit class ValueEncoderOps[A](val encoder: Encoder[A]) extends AnyVal {
    def ::[H] (pre: Encoder[H]): Encoder[H :: A :: HNil] = new Encoder[H :: A :: HNil]{
  ...
  implicit class HListEncoderOps[L <: HList](val encoder: Encoder[L]) extends AnyVal {
    ...
    def as[A](implicit gen: Generic.Aux[A, L]): Encoder[A] = encoder.contramap(l => gen.to(l) )
  }

  implicit class ValueDecoderOps[A](val Decoder: Decoder[A]) extends AnyVal {
    def ::[H] (pre: Decoder[H]): Decoder[H :: A :: HNil] = new Decoder[H :: A :: HNil]{
  ...
  implicit class HListDecoderOps[L <: HList](val decoder: Decoder[L]) extends AnyVal {
    ...
    def as[A](implicit gen: Generic.Aux[A, L]): Decoder[A] = decoder.map(l => gen.from(l) )
  }
}
